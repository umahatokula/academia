...
class Helper
{
	...
	public static function getFeeScheduleTotal($fee_schedule_code){
		...
		return DB::table($table)->where('fee_schedule_code', $fee_schedule_code)->sum('amount');
	}
	...
	Send mail
	 */
	public static function sendStudentInvoice($emailcontent, $student){
	    	// dd($student);
		...
		\Mail::send  (
			...
			function($message) use($emailcontent, $student){
				$message->to($emailcontent['parent_email'], 'Invoice')->subject($student->fname.'\'s Invoice');
			}
			);
	...
	public static function getParentDiscountEligibles () {
	    ...
	    public static function getParentDiscountAmount($fee_schedule_code){
	    	...
	    	if($discount_policy->type == 'percentage'){
	    		...
	    		foreach(json_decode($discount_policy->affected_elements) as $fee_element_id){
	    			$amount += DB::table($table)->where(['fee_schedule_code' => $fee_schedule_code, 'fee_element_id' => $fee_element_id])->first()->amount;
	    		}
	    ...
	    public static function calculateParentDiscount ($student_id, $fee_schedule_code) {
	    	...
	    	$same_parent = array_flatten($same_parent);
	    	...
	    	if($discount_policy->all_wards == 1){
        		...
        		//dont divide parent discount but spread accross all wards that have thesame parent
	    	if($discount_policy->dont_divide == 1){
        		...
        		//which of the wards to enjoy this discount
	    	if($discount_policy->all_wards == 0 && $discount_policy->dont_divide == 0){

	    		sort($same_parent);

	    		foreach ($same_parent as $key => $student_id) {
	    ...
	    public static function get_suffix($position) {
	    	...
	    	$position_last_digit = substr($position, -1, 1);
	    	$suffix = '';

	    	if ($position == 11 || $position == 12 || $position == 13) {
	    		return "th";
	    	} elseif ($position_last_digit == 1) {
	    		return "st";
	    	} elseif ($position_last_digit == 2 ) {
	    		return "nd";
	    	} elseif ($position_last_digit == 3 ) {
	    		return "rd";
	    	} else {
	    		return "th";
	    	}
